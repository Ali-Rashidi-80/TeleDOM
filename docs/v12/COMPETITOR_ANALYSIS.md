# TeleDOM v12 — Competitor & Ecosystem Analysis

> Positioning evidence for the v4→v12 transformation. Primary sources: official
...
> the implemented v12 capability — never a marketing counter-claim.

## Category: TeleDOM v12 = Temporal Browser Intelligence (new category)

| Technology | Strengths | Weaknesses (evidence) | TeleDOM gap (baseline) | TeleDOM v12 response |
|---|---|---|---|---|
...
| **Replay.io** | Deterministic RUNTIME replay (DOM + network + JS execution frames), time-travel debugging with agents | Closed-source infrastructure; requires forked browser runtime; heavyweight sessions | Baseline TeleDOM has DOM-level reconstruction only — runtime-level determinism is a genuine gap | v12 counterfactual engine gives event-level deterministic replay with causal propagation + explicit fidelity levels; full runtime replay honestly DEFERRED (documented limitation, not claimed) |
...
| Failure class (primary evidence) | Which competitors hit it | v12 structural answer |
|---|---|---|
...
explanation. TeleDOM v12 is built exactly against those constraints, defining
...
`docs/v12/V12_ENGINEERING_REPORT.md#limitations` (deterministic runtime replay,
