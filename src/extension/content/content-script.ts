...
(function () {
  ...
  // Direct WebSocket connection to MCP Bridge for immediate tab communication
  let directWs: WebSocket | null = null;
  let directReconnectTimer: any = null;

  function connectDirectBridge() {
    if (typeof WebSocket === 'undefined') return;
    try {
      const ws = new WebSocket('ws://127.0.0.1:3847');
      ws.onopen = () => {
        directWs = ws;
        console.log('[Forensic ContentScript] Connected directly to MCP Bridge on ws://127.0.0.1:3847');
        if (directReconnectTimer) {
          clearInterval(directReconnectTimer);
      ...
      ws.onmessage = async (event) => {
        try {
          ...
          if (message.type === 'BROWSER_COMMAND_REQUEST') {
            ...
            // 1. Background commands & Tab management
            if (['LIST_TABS', 'OPEN_TAB', 'LIST_EXTENSIONS', 'RELOAD_EXTENSION', 'CLOSE_TAB', 'FOCUS_TAB', 'RELOAD_TAB', 'RESIZE_VIEWPORT', 'RESET_VIEWPORT'].includes(command)) {
              if (typeof chrome !== 'undefined' && chrome.runtime?.sendMessage) {
                chrome.runtime.sendMessage(message, (res) => {
                  if (chrome.runtime.lastError) {
                    // Local fallback if background unreachable
                    if (command === 'CLOSE_TAB') {
                      ws.send(JSON.stringify({ type: 'BROWSER_COMMAND_RESPONSE', id, command, success: true, data: { closed: true, url: window.location.href, title: document.title } }));
                      setTimeout(() => window.close(), 100);
                      return;
                    }
                    if (command === 'RELOAD_TAB') {
                      ws.send(JSON.stringify({ type: 'BROWSER_COMMAND_RESPONSE', id, command, success: true, data: { reloaded: true, url: window.location.href, title: document.title } }));
                    ...
                    ws.send(
                      JSON.stringify({
                        ...
                        command,
                        success: false,
                        error: { code: 'FORWARD_ERROR', message: chrome.runtime.lastError.message },
                      })
                    );
                  } else {
            ...
            // 3. GET_TAB_CONSOLE_LOGS
            if (command === 'GET_TAB_CONSOLE_LOGS') {
            ...
            // 4. GET_TAB_NETWORK_REQUESTS
            if (command === 'GET_TAB_NETWORK_REQUESTS') {
            ...
            // 5. DOM Commands
            const res = await liveController.handleCommand(message, document);
            ws.send(JSON.stringify(res));
          }
        } catch (err: any) {
          console.error('[Forensic ContentScript] Bridge message error:', err);
        }
      };
    } catch {
      directWs = null;
  ...
  function ensureDirectReconnect() {
    if (!directReconnectTimer) {
      directReconnectTimer = setInterval(() => {
