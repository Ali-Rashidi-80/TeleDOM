...
export class MCPToolsHandler {
  ...
  constructor(storage: ForensicStorageProvider, liveToolsHandler?: LiveToolsHandler, extendedToolsHandler?: ExtendedToolsHandler) {
    ...
    // The extended handler SHARES the live handler's controller + bridge so
    // there is exactly one LiveBrowserController per server process.
    this.extendedToolsHandler =
  ...
  public getExtendedToolsHandler(): ExtendedToolsHandler {
  ...
  public async handleToolCall(name: string, args: Record<string, any>): Promise<MCPToolCallResult> {
    try {
      ...
      // MCPDOM v3 platform-evolution tools (74 tools)
  ...
  private async handleImportSession(args: Record<string, any>): Promise<MCPToolCallResult> {
    ...
    for (const chk of bundle.checkpoints) {
      // Remap checkpoint to the imported session so storage stays consistent.
      await this.storage.saveCheckpoint({ ...chk, sessionId: bundle.metadata.id });
    }
    for (const ann of bundle.annotations) {
      // CRITICAL FIX: annotations carry their ORIGINAL sessionId in the bundle.
      // Writing them unremapped corrupts the SOURCE session's annotations.json
      // (each import doubled the source annotations — O(n²) write growth).
