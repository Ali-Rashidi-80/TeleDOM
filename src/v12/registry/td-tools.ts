/**
 * TeleDOM v12+ Registry — MCP tool definitions GENERATED from the
...
export const TELEDOM_V12_TOOLS: MCPToolDefinition[] = CAPABILITY_REGISTRY.map((cap) => ({
  name: cap.id,
...
export const TD_TOOL_NAMES: Set<string> = new Set(TELEDOM_V12_TOOLS.map((t) => t.name));
