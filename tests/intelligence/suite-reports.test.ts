/**
 * TeleDOM v4 suite-reports: runs the REAL benchmark matrix, the golden
 * incident suite (1020 scenarios) and the chaos suite, asserts thresholds,
 * and GENERATES docs/intelligence/BENCHMARKS.md from the measured numbers.
 * Docs are never hand-written — they are test output.
 */
import { describe, it, expect } from 'vitest';
import fs from 'fs';
import path from 'path';
import { runBenchmarks } from '../../src/intelligence/bench/benchmarks';
import { generateGoldenSuite, runGoldenSuite } from '../../src/intelligence/golden/generator';
import { runChaosSuite } from '../../src/intelligence/chaos/chaos';

describe('v4 measured suites (benchmarks + golden + chaos)', () => {
  it('benchmark matrix: 10K / 100K / 1M events with p50/p95/p99', () => {
    ...
    // ---- Generate docs/intelligence/BENCHMARKS.md from measured values ----
    const outDir = path.resolve(process.cwd(), 'docs/intelligence');
    fs.mkdirSync(outDir, { recursive: true });
    let md = `# TeleDOM v4 — Measured Benchmarks & Suite Metrics\n\n`;
