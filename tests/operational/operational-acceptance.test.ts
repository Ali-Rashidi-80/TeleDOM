...
describe('MCP-DOM Complete Operational Acceptance & Certification Suite', () => {
  ...
  it('verifies dynamic MCP discovery matches exposed tools', () => {
    ...
    expect(discovered.length).toBeGreaterThanOrEqual(34);
    expect(FORENSIC_MCP_TOOLS.length).toBeGreaterThanOrEqual(34);
  });
  ...
  it('verifies every exposed tool has a dedicated operational evidence folder with raw JSON-RPC captures', () => {
    ...
    expect(toolFolders.length).toBeGreaterThanOrEqual(34);
  ...
  it('verifies visual screenshot tools contain valid PNG binaries and geometry metadata', () => {
    // Folder indexes shift as the tool catalog grows — resolve dynamically by name.
    const allFolders = fs.existsSync(TOOLS_DIR) ? fs.readdirSync(TOOLS_DIR) : [];
    const visualFolders = allFolders.filter((f: string) => /capture_(page|element)_screenshot$/.test(f));
    expect(visualFolders.length).toBe(2);
  ...
  it('verifies final certification reports and capability matrix exist and assert CERTIFIED status', () => {
    ...
    const expectedTotal = (globalThis as any).__EXPECTED_TOOL_COUNT__ ?? 121;
